...
function isInstalledSkillDir(dir) {
  const normalized = resolve(dir).replace(/\\/g, "/").toLowerCase();
  return normalized.endsWith("/skills/office-live") || normalized.includes("/skills/office-live/");
}

function defaultOutputRoot(skillRoot) {
  if (process.env.OFFICE_LIVE_OUTPUT_DIR) return resolve(process.env.OFFICE_LIVE_OUTPUT_DIR);
  if (isInstalledSkillDir(skillRoot)) return join(homedir(), "Documents", "Codex", "office-live-output");
  return join(skillRoot, "office-live-output");
}
...
try {
  ...
  try {
    ...
    if (!userAuth || !okStatuses.includes(userAuth.status)) {
      // 未授权：发起 Device Flow 授权，生成二维码图片供用户扫码（只扫 base 域=飞书表格读写）
      console.log("未检测到飞书表格授权。正在生成授权二维码…");
      const loginRaw = run(
        larkCmd("auth login --domain base --no-wait --json"),
        { timeout: 30000 },
      );
    ...
    if (String(e.message || e).includes("授权")) throw e;
    // auth status 本身异常（如 lark-cli 不可用）：继续，让后续命令暴露真实问题
    console.log("（登录状态检查跳过：" + (e.message || e).toString().slice(0, 120) + "）");
  }
  ...
  const dueName = hint(dateSem, ["截止", "结束", "Due", "Deadline", "End", "到期", "交付", "完成日期"]);
...
`;
  ...
  for (let i = 0; i < 6; i++) {
    if (existsSync(join(skillRoot, "templates", "page", "lib", "office-config.ts"))) break;
    const parent = resolve(skillRoot, "..");
    if (parent === skillRoot) break;
    skillRoot = parent;
  }
  const outDir = defaultOutputRoot(skillRoot);
  try {
