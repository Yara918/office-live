...
const DEFAULT_OFFICE_SPAWNS = [
  { seatId: "seat-0", x: 418.9, y: 647.19, facing: "right" as const },
  { seatId: "seat-1", x: 264.172, y: 335.842, facing: "down" as const },
  { seatId: "seat-2", x: 408.355, y: 459.566, facing: "up" as const },
  { seatId: "seat-3", x: 408.054, y: 334.674, facing: "down" as const },
  { seatId: "seat-4", x: 551.97, y: 336.339, facing: "down" as const },
  { seatId: "seat-5", x: 263.373, y: 458.687, facing: "down" as const },
  { seatId: "seat-6", x: 551.97, y: 459.396, facing: "down" as const },
];

function createDefaultOfficeSeats(): SeatState[] {
  // 座位默认全部为空，等待飞书快照（OfficeRosterSync）填充
  return DEFAULT_OFFICE_SPAWNS.map((spawn) => ({
    seatId: spawn.seatId,
    label: `工位 ${spawn.seatId}`,
    seatType: "worker",
    roleTitle: undefined,
    assigned: false,
    spriteKey: undefined,
...
export function mergeDiscoveredSeats(
  ...
  return discovered.map((seat, index) => {
    ...
    // 座位默认全部为空，等待飞书快照（OfficeRosterSync）填充，不使用演示数据
    const assigned = stored?.assigned ?? false;
    const spriteKey = stored?.spriteKey;
    const spritePath = stored?.spritePath;
    const label = stored?.label ?? `工位 ${index + 1}`;
    const seatType: SeatType = stored?.seatType ?? "worker";
    const roleTitle = stored?.roleTitle;
