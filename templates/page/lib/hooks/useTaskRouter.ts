...
export function useTaskRouter(refs: TaskRouterRefs) {
  ...
  const runDemoTask = useCallback(
    (taskId: string, message: string, seatId?: string) => {
      const actorName = resolveSeatLabelForTask(refs.seats.current, seatId);
      refs.dispatch.current({
        type: "UPDATE_TASK",
        taskId,
        patch: { status: "running", runId: taskId, actorName, seatId },
      });
      refs.dispatch.current({ type: "BIND_SEAT_RUN", taskId, runId: taskId });
      gameEvents.emit("task-bound", taskId, taskId);

      window.setTimeout(() => gameEvents.emit("task-bubble", taskId, "收到，回工位处理", 2600), 300);
      window.setTimeout(() => gameEvents.emit("task-bubble", taskId, "正在推进：" + message.slice(0, 12), 3000), 2800);
      window.setTimeout(
        () => gameEvents.emit("task-bubble", taskId, "已进入处理中，等待表格状态更新", 3600),
        7200,
      );
    },
    [refs],
    ...
    (message: string, seatId?: string) => {
      ...
      refs.dispatch.current({
        ...
        task: {
          taskId,
          message,
          status: client && client.status === "connected" ? "submitted" : "queued",
          sessionKey,
          seatId,
          actorName,
  ...
  useEffect(() => {
    return gameEvents.on("task-ready", (taskId, message, seatId) => {
      ...
      if (!client || client.status !== "connected") {
        runDemoTask(taskId, message, seatId);
        return;
      }
  ...
  }, [runDemoTask, sendTaskToGateway, refs]);
