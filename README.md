...
- **本地运行**：页面运行在 agent 环境，默认优先使用 `http://localhost:3000`；如端口被占用，按 `deliver.mjs` 输出的新端口访问。不部署到外部平台；页面以「用户级登录自启」方式启动（Windows 登录触发计划任务 / macOS launchd LaunchAgent，见 deployment.md「启动页面」），用户登录电脑自动运行、无窗口、不被回收。
...
© 2026 Yara918. All rights reserved.
