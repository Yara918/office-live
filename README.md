...
- **本地运行**：页面运行在 agent 环境，默认优先使用 `http://localhost:3000`；如端口被占用，按 `deliver.mjs` 输出的新端口访问。不部署到外部平台；每次使用需启动页面（用「隐藏窗口 + 独立进程」方式：Windows VBScript / macOS launchd，见 deployment.md「启动页面」）。
